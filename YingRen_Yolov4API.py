...
class Yolov4YingRen():
    ...
    def catchObject_J(self, img, min_score_thresh, visualize_boxes_IO="OFF"):#回傳image 物件list
        ...
        if visualize_boxes_IO == 'ON':  # 判斷FastRCNN內建可試化視窗開啟或關閉
            ...
            for label, confidence, bbox in detections:
                ...
                cv2.rectangle(frame_rgb, (xmin, ymin), (xmax, ymax), self.class_colors[label], 1)
                cv2.putText(frame_rgb, "{} {:.2f}%".format(label, float(confidence)),
                    (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.class_colors[label], 2)
        ...
        return image, objs
    def Autolabel_YingRen(self, img, min_score_thresh, visualize_boxes_IO="OFF"):#回傳image 物件list
        if type(img) == str:  # 給自串路徑
        #    img = cv2.imread(img)
            image = img.copy()
        #    img1 = cv2.resize(img, (608, 608))
        ...
        for label, confidence, bbox in detections:
            ...
            # obj.append(label) # label name
            # obj.append(np.array([left, top, right, bottom])) # 座標
            # objs.append(obj) 
            xmin = round(float((left/self.width)),6)
            ymin = round(float((top/self.height)),6)
            xmax = round(float((right/self.width)),6)
            ymax = round(float((bottom/self.height)),6)
            
            x_center=round((xmin+ (0.5*(xmax-xmin))),6)
            y_center=round((ymin+ (0.5*(ymax-ymin))),6)
            bb_width=round(xmax-xmin,6)
            bb_height=round(ymax-ymin,6)
            #
            # obj.append(label) # label name
            # obj.append([xmin, ymin, xmax, ymax]) # 座標
            obj.append(label)
            obj.append(x_center)
            obj.append(y_center)
            obj.append(bb_width)
            obj.append(bb_height)            
            # obj.append(np.array([xmin, ymin,xmax - xmin, ymax - ymin])) # w, h
            # obj.append(float(confidence)) # 信心值
            objs.append(obj)            
